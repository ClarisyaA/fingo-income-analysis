...
## Pertanyaan untuk Mentor Data Science

### Validasi Metodologi

**1. Apakah distribusi Log-Normal + AR(1) sudah cukup untuk memodelkan income time-series gig worker Indonesia?**
Notebook menggunakan Log-Normal stasioner dengan AR(1) di log-space. Apakah perlu mempertimbangkan distribusi yang lebih kompleks (misalnya mixture model untuk mencerminkan gig worker yang ganti platform)?

**2. Apakah AR1_COEF = 0.45 yang menghasilkan lag-1 AC ≈ 0.35-0.40 sudah valid secara statistik untuk kasus ini?**
Target range 0.20–0.60 diambil dari literatur mana? Apakah ada benchmark empiris autokorelasi income gig worker dari studi serupa?

**3. Apakah konversi menggunakan faktor PPP (INDIA_ADAPT = 0.55) sudah tepat secara metodologis?**
Faktor 0.55 diturunkan dari perbandingan data World Bank PPP. Apakah ada cara yang lebih robust untuk mengkalibrasi faktor adaptasi ini, misalnya menggunakan data survei lapangan langsung?

**4. Apakah Bias Test 2 (KS Test) yang sering WARN masih dapat diterima?**
KS Test sering gagal karena seasonal multiplier menggeser distribusi dari Log-Normal teoritis. Apakah ada uji distribusi yang lebih tepat untuk data yang memiliki seasonal shock (misalnya goodness-of-fit test conditional per seasonal period)?

### Validasi Data & Sampling

**5. Apakah 300 user × 52 minggu (15.600 baris) sudah sufficient untuk training LSTM income prediction?**
Secara rule of thumb, LSTM membutuhkan minimal 1.000+ sequence. Dengan 300 user masing-masing 52 minggu, apakah ini cukup? Atau perlu augmentasi data lebih lanjut?

**6. Apakah chronological split 69%/15%/15% (minggu 1-36 / 37-44 / 45-52) sudah tepat?**
Split ini tidak memperhatikan bahwa minggu 45-52 mencakup Harbolnas dan yearend (peak season). Apakah tidak sebaiknya test set mencakup semua seasonal periods agar evaluasi lebih representatif?

**7. Bagaimana pendapat Mentor tentang proporsi data sintetis vs survei?**
Jika survei menghasilkan < 50 responden valid, itu berarti > 94% data sintetis. Apakah threshold minimum proporsi data real yang perlu dicapai sebelum model bisa diklaim cukup andal?

### Validasi Feature Engineering

**8. Apakah lag_1w, lag_2w, lag_4w sebagai fitur sudah mencukupi, atau perlu menambah lag yang lebih panjang (lag_12w, lag_26w)?**
Untuk LSTM yang belajar sendiri dari sequence, apakah lag feature eksplisit ini perlu, atau justru redundan?

**9. Apakah MinMaxScaler per user (bukan per gig_type) adalah pilihan yang tepat?**
Normalisasi per user memungkinkan model belajar pola relatif (naik-turun), bukan nilai absolut. Apakah ini konsisten dengan target prediksi yang nantinya harus di-inverse transform untuk mendapat nilai IDR sebenarnya?

**10. Apakah `income_volatility` (CoV global per user selama 52 minggu) layak dipakai sebagai fitur input model?**
Fitur ini menggunakan seluruh 52 minggu untuk dihitung, artinya ada data leakage untuk minggu-minggu awal (model "tahu" volatilitas masa depan). Apakah ini harus diganti dengan rolling CoV saja?
